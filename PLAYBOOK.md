...
│   ├── skills/                        # 7 built-in execution skills
...
│   │   ├── ship/                     #   Full ship automation
│   │   ├── investigate/              #   Root-cause debugging
│   │   ├── document-release/         #   Post-ship doc sync
│   │   └── safety/                   #   Careful, freeze, guard modes
...
#### Bước 2a: CEO Review — "Đây có phải thứ cần build?"
...
#### Bước 2b: Eng Review — "Architecture, diagrams, edge cases"
...
### Phase 4: Paranoid Review — "Staff Engineer mode"
...
### Phase 5: Ship — "Không nói thêm, ship it"
...
## Skill Architecture — Local Skills

### Core Rule: Token Budget

Tất cả skills nằm tại `.claude/skills/`. **KHÔNG có external dependencies.**

```
LUÔN ÁP DỤNG:
  ✓ Chỉ load SKILL.md khi user gõ command hoặc Master-Agent route đến
  ✗ KHÔNG load 2 SKILL.md cùng lúc
...
```
Mỗi gate phải pass trước khi sang phase tiếp. `/gsd` bypass tất cả gates cho tasks < 15 min.
Gate status tracked trong `.prism/GATE_STATUS.md`.

### Workflow Chain (typical feature cycle)

```
Command                  Skill                         Knowledge Store
───────                  ─────                         ──────────────
/brainstorm         →    (inline)
/ceo-review         →    .claude/skills/ceo-review/    →   .prism/designs/
/eng-review         →    .claude/skills/eng-review/    →   .prism/designs/
/plan → GO          →    [sub-agents implement]
/paranoid-review    →    (inline)                      →   .prism/knowledge/
/review             →    .claude/skills/code-review/   →   .prism/knowledge/
/ship               →    .claude/skills/ship/
/document-release   →    .claude/skills/document-release/ → .prism/knowledge/
/investigate        →    .claude/skills/investigate/   →   .prism/knowledge/
/office-hours       →    (inline)                      →   .prism/brainstorms/
/careful,/freeze    →    .claude/skills/safety/        →   (session-scoped)
```

### Output → .prism/ Integration

Mọi skill output PHẢI được lưu vào .prism/ knowledge system:

```
/ceo-review       → .prism/designs/ceo-review_{topic}_{date}.md
/eng-review       → .prism/designs/eng-review_{topic}_{date}.md
/review findings  → .prism/knowledge/GOTCHAS.md (append)
/qa-only report   → .prism/qa-reports/qa_{date}.md
...
/office-hours     → .prism/brainstorms/office-hours_{topic}_{date}.md
```
