...
Read `.claude/skills/ship/test-coverage-audit.md` and apply its 6-step audit:
trace codepaths → score existing tests → map user flows → ASCII coverage diagram →
generate tests for uncovered paths (max 10) → summary line for PR body.

**Skip conditions:**
- Diff is test-only → "No new application code paths to audit." Continue.
- No test framework → diagram only, no generation.

**Output for PR body (always):**
