...
Read `.claude/skills/ceo-review/sections.md` and apply each section **in order**:

1. **Architecture** — components, data flow, scaling, failure scenarios, rollback
2. **Error & Rescue Map** — every failure path named, rescued, tested
3. **Security & Threat Model** — attack surface, authz, secrets, injection vectors
4. **Data Flow & Interaction Edge Cases** — shadow paths for every flow
5. **Code Quality** — DRY, naming, over/under-engineering
6. **Test Review** — coverage diagram, ambition check, flakiness
7. **Performance** — N+1, memory, indexes, p99 latency
8. **Observability** — logging, metrics, alerts, runbooks
9. **Deployment & Rollout** — migrations, flags, rollback plan
10. **Long-Term Trajectory** — debt, path dependency, reversibility score
11. **Design & UX** — skip if no UI scope detected

**Rules for every section:**
- End with **STOP** — AskUserQuestion once per issue, do NOT batch
- Do NOT proceed until user responds
- EXPANSION / SELECTIVE EXPANSION modes have additional prompts per section (see sections.md)
